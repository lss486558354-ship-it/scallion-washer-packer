# -*- coding: utf-8 -*-
# Single-pass modification script for main.c
# This script makes ALL changes in one pass to maintain accuracy
import sys
...
# Verify brace balance of backup
...
# ============================================================
# STEP 2: Replace PACK_TEST trigger (around line 346)
# ============================================================
...
# ============================================================
# STEP 3: Delete old partition H boot auto-run (lines 267-274)
# ============================================================
for i in range(len(lines)):
    if 's_app_tick_armed = 1u;' in lines[i].strip():
        for j in range(i + 1, min(i + 20, len(lines))):
            if '/* 确保 PA9/PA10' in lines[j]:
                del lines[i+1:j]  # Delete lines i+1 to j-1
                print(f"STEP 3: Deleted partition H boot auto-run (lines {i+2} to {j})")
                break
        break

# ============================================================
# STEP 4: Replace state machine (find boundaries by markers)
# ============================================================
# Find start: line with '【分区 D】新工艺流程主状态机'
...
# Find end: line with '【分区 E】超声波检测计数'
e_start = None
for i, line in enumerate(lines):
...
if d_start and e_start:
    print(f"STEP 4: State machine block to replace: lines {d_start+1} to {e_start}")
else:
    print(f"ERROR: Could not find state machine. d_start={d_start}, e_start={e_start}")
    sys.exit(1)

# New state machine code (carefully balanced)
new_block = r'''    /* ========================================================================
     * 【分区 F】切割+打包联合状态机
     *
...
'''
    ...
    # Rebuild: keep lines 0 to d_start-1, add new_block, skip d_start to e_start-1
    lines = lines[:d_start] + [new_block] + lines[e_start:]
    print(f"STEP 4: Replaced state machine lines {d_start+1}-{e_start} with new block")

# ============================================================
# STEP 5: Delete old partition E (ultrasonic counting, after new block)
# ============================================================
for i in range(len(lines)):
    if '【分区 E】超声波检测计数' in lines[i]:
        e_start = i
        for j in range(i, len(lines)):
            if '【分区 F】USART3 0x9A' in lines[j]:
                del lines[e_start:j]  # Delete lines e_start to j-1
                print(f"STEP 5: Deleted partition E (lines {e_start+1}-{j})")
                break
        break

# ============================================================
# STEP 6: Delete old partition H state machine (find by marker)
# ============================================================
for i in range(len(lines)):
    if '【分区 H】打包测试状态机' in lines[i]:
        for j in range(i - 1, -1, -1):
            if '/* =' in lines[j] and '===' in lines[j]:
                h_start = j
                break
        ...
        if h_end:
            ...
            print(f"STEP 6: Deleted partition H state machine (lines {h_start+1}-{h_end})")
        else:
            print(f"WARNING: Could not find end of partition H (brace_count={brace_count})")
        break

# ============================================================
# STEP 7: Delete old partition G/USART3 trigger (now redundant)
# ============================================================
for i in range(len(lines)):
    if '【分区 F】USART3 0x9A' in lines[i]:
        for j in range(i, min(i + 10, len(lines))):
            if 'g_protocol_process_start_request' in lines[j]:
                # Delete just the if block content (keep comment)
                lines[j] = '        /* request handled in PROC_NEW_IDLE case */\n'
                print(f"STEP 7: Cleared redundant USART3 trigger at line {j+1}")
                break
        break
...
raw_out = open(main_path, 'rb').read()
ob = raw_out.count(b'{')
cb = raw_out.count(b'}')
oc = raw_out.count(b'/*')
cc = raw_out.count(b'*/')
print(f"\nFinal: {len(lines)} lines")
print(f"Braces: {{={ob} }}={cb} balance={ob-cb}")
print(f"Comments: /*={oc} */={cc} balance={oc-cc}")
